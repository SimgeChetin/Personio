...
, revenue_generated as (
    select
        ...
        , coalesce(lag(revenue_cash) over (partition by cust_subs_monthly.customer_id, cust_subs_monthly.subscription_id order by cust_subs_monthly.billing_year_month asc ),0) as starting_month_revenue_cash
...
select
    ...
    ---- if the subscription is not new --> starting_month_revenue_after_discounts + upgrade_revenue - downgrade_revenue - churn_lost_revenue_after_discounts(if custounts has churned)
    , case when is_new_subscription then 
